...
contract TiempoCVXBondDepository is Ownable {
    ...
    address public immutable treasury; // mints TIFI when receives principal
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "CVX Bond too small" ); // must be > 0.01 TIFI ( underflow protection )
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( TIFI ).safeTransfer( _recipient, _amount ); // send payout
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principal or TIFI) to the DAO
     *  @return bool
     */
