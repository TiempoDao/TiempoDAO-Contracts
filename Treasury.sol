...
contract TiempoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STIFI }
    ...
    address public sTIFI;
    uint public sTIFIQueue; // Delays change to sTIFI address
    ...
    /**
        @notice allow approved address to deposit an asset for TIFI
        @param _amount uint
...
//        
        ...
        // mint TIFI needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TIFI ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TIFI for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITIFIERC20( TIFI ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTIFI ).balanceOf( msg.sender ); // Can only borrow against sTIFI held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TIFI
        @param _amount uint
     */
    function repayDebtWithTIFI( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITIFIERC20( TIFI ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TIFI, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TIFI ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TIFI ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TIFI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TIFI decimals
            value_ = _amount.mul( 10 ** IERC20( TIFI ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STIFI ) { // 9
            sTIFIQueue = block.number.add( blocksNeededForQueue );
        } else return false;
