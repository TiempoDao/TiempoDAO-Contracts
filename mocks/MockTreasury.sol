...
contract MockTiempoTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sTIFIQueue; // Delays change to sTIFI address
  ...
  constructor(
    ...
    isLiquidityToken[ _TIFIDAI ] = true;
    liquidityTokens.push( _TIFIDAI );
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sTIFI).balanceOf(msg.sender); // Can only borrow against sTIFI held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithTIFI(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(TIFI).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.STIFI) {
      // 9
